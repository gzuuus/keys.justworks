...
describe("KeyholderCore lifecycle", () => {
  ...
  it("unlock → getPublicKey → lock", async () => {
    const core = new KeyholderCore();
    expect(await ok(core, msg("unlock", { ncryptsec: NCRYPTSEC, identifier: ID, password: PW }))).toEqual({
      pubkey: PUBKEY,
    });
...
describe("KeyholderCore CPU offload", () => {
  it("passwordSecret returns the golden auth secret, statelessly", async () => {
    const core = new KeyholderCore();
    const secret = (await ok(core, msg("passwordSecret", { identifier: ID, password: PW }))) as string;
...
describe("KeyholderCore NIP-07 signing", () => {
  it("signEvent adds id, pubkey, sig and verifies", async () => {
    ...
    const event = (await ok(
      core,
      msg("signEvent", { event: { kind: 1, content: "hello from the worker", tags: [], created_at: 1_700_000_0000 } }),
    )) as Event;
...
describe("KeyholderCore nip04 / nip44", () => {
  it("nip04 encrypt → decrypt round-trips (to self)", async () => {
    const core = new KeyholderCore();
    await ok(core, msg("unlock", { ncryptsec: NCRYPTSEC, identifier: ID, password: PW }));
    const enc = (await ok(core, msg("nip04.encrypt", { pubkey: PUBKEY, plaintext: "secret msg" }))) as string;
    const dec = (await ok(core, msg("nip04.decrypt", { pubkey: PUBKEY, ciphertext: enc }))) as string;
  ...
  it("nip44 encrypt → decrypt round-trips (to self)", async () => {
    ...
    const dec = (await ok(core, msg("nip44.decrypt", { pubkey: PUBKEY, ciphertext: enc }))) as string;
