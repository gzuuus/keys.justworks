...
<div class="mx-auto max-w-3xl px-4 py-12">
	...
	<div class="mt-6">
		<p class="text-xs font-bold tracking-[0.2em] text-mint-deep uppercase">Remote signer</p>
		<h1 class="mt-2 text-3xl font-black tracking-tight">Bunker</h1>
		<p class="mt-2 max-w-2xl text-muted-foreground">
			Turn this tab into a Nostr Connect bunker. A client imports a bunker URI (or hands you a
			nostrconnect URI) and sends signing requests over a relay; this page signs them with the key
			held in the Worker. Each slot keeps a stable transport identity, so reloading reconnects your
			clients automatically once you unlock. The tab must stay open and unlocked — closing or idling
			drops the key.
		</p>
	</div>
	...
	{:else}
		{#if bunker.autoApprove}
			<Alert class="mt-4 border-sun/40 bg-sun/10 text-[#8a5e10]">
				<TriangleAlert class="size-4" />
				<AlertTitle>Auto-approve is on</AlertTitle>
				<AlertDescription>
					Every connect / sign / encrypt request is accepted automatically across all slots. Only
					run this against relays and clients you trust — a connected client can sign anything as
					your key until you stop the slot or the key locks.
				</AlertDescription>
			</Alert>
		{/if}
						...
						<CardContent class="flex flex-col gap-4">
							...
							<div class="flex flex-wrap items-center gap-2">
								{#if status(app) === 'stopped'}
									...
									>
								{:else}
									<Button variant="outline" size="sm" onclick={() => bunker.stop(app.id)}
										>Stop</Button
									>
								{/if}
								<Button
